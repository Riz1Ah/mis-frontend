...
function App() {
  ...
  return (
    <>
    {alertState===1 && <Alert children="What Have You Done!!" onClickClose={onClickCloseHandler}/>}
  <Button children="Dont Click Here" color={"primary"} onclickHandle={onClickHandler} />
  <ListGroup items={["London", "Paris", "NY", "LA", "San Fransisco"]} heading={"Cities"} onSelectItem={handleOnSelect}/> 

    <button onClick={useImmer}> ImmerUsage</button>
    {immerVar.map(iv=> <p key={iv.id}>{iv.id} {iv.bug? 'Fixed': 'Bug'}</p>)}
  </>
)
